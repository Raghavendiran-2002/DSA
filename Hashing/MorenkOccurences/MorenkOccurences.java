...
public class MorenkOccurences {
  ...
  static void printNByKNaive(int arr[], int n, int k) {
    Arrays.sort(arr);
    int i = 1, count = 1;
    while (i < n) {
      ...
      if (count > n / k) System.out.print(arr[i - 1] + " ");
      count = 1;
      i++;
    }
  }
  ...
  static void printNByKEff(int arr[], int n, int k) {
    ...
    for (Map.Entry<Integer, Integer> e : m.entrySet()) if (
      e.getValue() > n / k
